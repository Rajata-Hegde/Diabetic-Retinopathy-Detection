...
function AnalyticsPage({ records }) {
  ...
  return (
    ...
    >
      ...
      <motion.div variants={item} className="relative h-[320px] w-full rounded-[48px] overflow-hidden group">
        <img 
          src="/assets/analytics_hero.png" 
          className="h-full w-full object-cover opacity-50 group-hover:scale-105 transition-transform duration-[5s]" 
          alt="Analytics Hero"
        />
        ...
        <div className="absolute inset-0 p-12 flex flex-col justify-center">
           <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-[10px] font-black uppercase tracking-[0.3em] text-sky-400">Advanced Bio-Metrics</span>
           </div>
           <h2 className="text-5xl font-black text-white tracking-tighter">Cohort Intelligence</h2>
           <p className="mt-4 text-lg text-slate-400 font-medium max-w-md leading-relaxed">
             Deep-dive into diagnostic distributions, model confidence trends, and clinical risk tiers across your active screening vault.
           </p>
        </div>
        ...
        <div className="absolute right-12 bottom-12 flex gap-4">
           {insights.map((_, i) => (
             <button 
              ...
              className={`h-1.5 rounded-full transition-all duration-500 ${activeInsight === i ? 'w-12 bg-sky-500' : 'w-2 bg-white/20'}`}
             />
           ))}
        </div>
      </motion.div>

      {/* Insight Highlight Carousel (Cards) */}
      <div className="grid gap-6 md:grid-cols-3">
         {insights.map((insight, i) => (
           <motion.div
            ...
            variants={item}
            whileHover={{ y: -10 }}
            className={`premium-glass p-8 rounded-[40px] border border-white/5 bg-gradient-to-br ${insight.color} relative overflow-hidden group`}
           >
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-8">
                  <div className="h-12 w-12 rounded-2xl bg-white/10 flex items-center justify-center backdrop-blur-xl border border-white/10 group-hover:scale-110 transition-transform">
                    {insight.icon}
                  </div>
      ...
      <div className="grid gap-10 lg:grid-cols-2">
        ...
        <motion.div variants={item} className="premium-glass p-10 rounded-[48px] border border-white/5 relative group">
        ...
        <motion.div variants={item} className="premium-glass p-10 rounded-[48px] border border-white/5 group">
          <div className="flex items-center justify-between mb-10">
            <div>
